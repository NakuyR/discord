...
async def 들어와(ctx):
    try:
        ...
        try:
            ...
            await ctx.send("음성 채널에 접속해서 명령어를 입력해주세요")
...
async def 나가(ctx):
    try:
        ...
        await ctx.send("그 음성채널에 들어가있지 않아요")
...
async def 재생(ctx, *, msg):
    ...
    if not vc.is_playing():
        ...
        await ctx.send("대기열에"+result+"을(를) 추가했습니다.")   
...
bot.run('OTIwNjgxNTgyOTI5NTIyNzU4.Ybn5ig.yyWgL0pyIkf7SRrgtIt71jkGljE')
